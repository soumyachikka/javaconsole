...
class Visitor extends React.Component{
    ...
    visitIdentifier(node) {
        ...
        return (globalScope.get(name)) ? globalScope.get(name) : 
    ...
    visitBinaryExpression(node) {
        const leftNode = isNaN(this.visitNode(node.left)) ?
             this.visitNode(node.left) : +(this.visitNode(node.left));
        const operator = node.operator
        const rightNode = isNaN(this.visitNode(node.right)) ? 
            this.visitNode(node.right) : +(this.visitNode(node.right));
    ...
    visitCallExpression(node) {
        ...
        const _arguments = this.evalArgs(node.arguments, callee);
